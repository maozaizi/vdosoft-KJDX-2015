...
<script type="text/javascript">
	...
	function update(){
		...
		document.myform.action="${pageContext.request.contextPath}/api/project/updateProject";
...
<div class="container-fluid">
  ...
  <input type="hidden" id="url" name="url" value="${pageContext.request.contextPath}/api/project/list"/>
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <input type="text" class="form-control" id="projectname" name="projectname" value="${project.projectname}" placeholder="项目名称">
      </div>
    </div>
    <div class="form-group">
      <label for="inputEmail3" class="col-sm-2 control-label">项目编号</label>
      <div class="col-sm-10">
      	<c:if test="${project.istemp=='1'}">
        	<input type="text" class="form-control" id="projectcode" name="projectcode" value="${project.projectcode}" placeholder="项目编号">
      	</c:if>
      	<c:if test="${project.istemp=='0'}">
        	${project.projectcode}
        	<input type="hidden" id="projectcode" name="projectcode" value="${project.projectcode}" placeholder="项目编号">
      	</c:if>
      </div>
    </div>
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <input type="text" class="form-control" id="builddate" name="builddate" value="${project.builddate}" placeholder="立项时间">
      </div>
    </div>
    <div class="form-group">
      <label for="inputEmail3" class="col-sm-2 control-label">负责人单位</label>
      <div class="col-sm-10">
        <input type="text" class="form-control" id="college" name="college" value="${project.college}" placeholder="负责人单位">
      </div>
    </div>
    <div class="form-group">
      <label for="inputEmail3" class="col-sm-2 control-label">负责人</label>
      <div class="col-sm-10">
        <input type="text" class="form-control" id="dutyperson" name="dutyperson" value="${project.dutyperson}" placeholder="负责人">
      </div>
    </div>
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <select class="form-control" id="feeType" name="feeType">
          <option value="6" <c:if test="${project.feeType =='6' }">selected</c:if>>6</option>
          <option value="5" <c:if test="${project.feeType =='5' }">selected</c:if>>5</option>
        </select>
      </div>
    </div>
    <div class="form-group">
        ...
        <select class="form-control" name="projectLevel" id="projectLevel" onchange="getplantype(this[selectedIndex].value);">
        	<option value="">请选择</option>
          	<c:forEach var="projectLevel" items="${web:getDataItem('projectLevel')}" >
	        	<option value="${projectLevel.dataItemId}" <c:if test="${projectLevel.dataItemId ==project.projectlevelid }">selected</c:if>>${projectLevel.dataItemName}</option>
		 	</c:forEach>
        </select>
      </div>
    </div>
        ...
        <select class="form-control" id="planType" name="planType" onchange="hiddenText();">
          ...
          <c:if test="${!empty planTypeList}">
    			<c:forEach items="${planTypeList}" var="plan">
    				<option value="${plan.dataitemid}_${plan.p2}" <c:if test="${plan.dataitemid ==project.plantypeid }">selected</c:if>>${plan.dataitemname}</option>
    			</c:forEach>
    		</c:if>
        </select>
      </div>
    ...
    <div class="form-group">
      ...
      <div class="col-sm-10">
      	<input type="text" class="form-control" id="otherType" name="otherType" value="${project.othertype}" placeholder="请在此输入计划类别" <c:if test="${project.isother =='0' }">readonly="true"</c:if>>
      </div>
    ...
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <input type="text" class="form-control" id="amount" name="amount" value="${project.amount}" placeholder="到款金额" onblur="changefee();">
      </div>
    </div>
    ...
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <input type="text" class="form-control" id="managefee" name="managefee" value="${project.managefee}" placeholder="管理费">
      </div>
    </div>
    ...
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <input type="text" class="form-control" id="keyanfee" name="keyanfee" value="${project.keyanfee}" placeholder="科研费">
      </div>
    </div>  
    ...
    <div class="form-group">
        ...
        <div class="col-sm-10">
            <input type="text" class="form-control" id="pzNo" name="pzNo" value="${project.pzNo}" placeholder="凭证编号">
        </div>
    </div>
